...
4.REV4板上传代码成功后，连接蓝牙，上电后，蓝牙模块上LED闪烁。点击APP![](media/93bbea5cdfbd12d0d11a9a8e85d33b0e.png)图标，搜索到蓝牙，显示如下图。
...
5.点击桌面小车的![](media/1ec777d4324b73a9faa0973f0beb90f1.png)
图片按钮，进入控制桌面小车的界面
...
两电机分别对应的连接到电机驱动扩展板上的接口A和接口B；蓝牙模块的RXD、TXD、GND、VCC分别对应的接到电机驱动扩展板上的TX、RX、-（GND）、+（VCC），而蓝牙模块的STATE和BRK两引脚不需要接，电源接到BAT接口。
