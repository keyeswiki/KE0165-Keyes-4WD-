...
.. image:: media/ae80c4ac2d601bf0e96ccb25ed872dcb.jpg
...
+-----------------------+-----------------------+-----------------------+
| keyes UNO R3 for      | Keyes brick L298P     | keyes                 |
| arduino 开发板*1      | 电机驱动扩展板 V1*1   | 草帽LED白发红模块*1   |
+=======================+=======================+=======================+
...
.. image:: media/404ab1579bef4c98dfdc2aa1530c7787.png
...
  怎么样是不是很好理解，就是通过改变delay这个代码的时间，来改变9脚LED亮和灭的频率，不多说，我们上传代码。看看这个LED灯闪烁的频率是不是比之前快了？
...
.. image:: media/a6dbf9a1d18d8849437f013dcaf2d4a6.png
...
+-------------+------------------------------------------+-------------+
| keyes UNO   | Keyes brick L298P 电机驱动扩展板 V1*1    | keyes       |
| R3 for      |                                          | 草帽LED白   |
| arduino     |                                          | 发红模块*1  |
| 开发板*1    |                                          |             |
+=============+==========================================+=============+
...
.. image:: media/8d32c66be794f5c422e946ea3500aa81.png
...
  再看下for里面，涉及一个新函数 **analogWrite()**\ 。

  我们知道数字口只有\ **0**\ 和\ **1**\ 两个状态，那如何发送一个模拟值到一个数字引脚呢？就要用到该函数。观察一下Arduino板，查看数字引脚，你会发现其中6个引脚旁标有“~”，这些引脚不同于其他引脚，它们可以输出PWM信号。

函数格式如下：

   analogWrite(pin, value)

**analogWrite()**\ 函数用于给PWM口写入一个0\ :sub:`255的模拟值。所以，value是在0`\ 255之间的值。特别注意的是，\ **analogWrite()**
函数只能写入具有\ **PWM**\ 功能的数字引脚，也就是\ **D3，D5，D6，D9，D10，D11**\ 引脚。
...
  上图绿色竖线代表方波的一个周期。每个\ **analogWrite(value)**\ 中写入的value都能对应一个百分比，这个百分比也称为占空比\ **(Duty
Cycle)**\ ，指的是高电平在周期内占的时间比值，也就是：占空比 =
高电平时间
...
  循迹传感器实际上是红外传感器。
此处使用的组件是TCRT5000红外管。其工作原理是利用红外光对颜色的不同反射率，然后将反射信号的强度转换为电流信号。在检测过程中，黑色在高电平时处于活动状态，而白色在低电平时处于活动状态。检测高度为0~3厘米。
...
+-----------------+-----------------+-----------------+-----------------+
| keyes UNO R3    | Keyes L298P     | keyes LED模块*1 | Keyes           |
| 开发板*1        | 电机驱动扩展板  |                 | 循迹传感器*1    |
|                 | \*1             |                 |                 |
+=================+=================+=================+=================+
...
.. image:: media/656e36875efe90bd1ea216206c2407dd.png
...
.. image:: media/440330dd5cfe3998b1db73a60d4e77e3.png
...
.. image:: media/415a1937ec707da81173cc3da9ab2ea0.png
...
.. image:: media/69be958142b773acdae33eeef12afed7.png
...
.. image:: media/c9173ab0ea434f3359aff23a2469be4f.png

  对应的舵机角度值如下:

.. image:: media/586b7664671a400f37588edb56d6cec9.png
...
+-----------------+-----------------+-----------------+-----------------+
| keyes UNO R3    | Keyes brick     | Keyes SG90 9G   | Keyes SG90 9G   |
| for arduino     | L298P           | 舵机*1          | 舵机*1          |
| 开发板*1        | 电机驱动扩展板  |                 |                 |
|                 | V1*1            |                 |                 |
+=================+=================+=================+=================+
| |image23|       | |image24|       | |image25|       |                 |
+-----------------+-----------------+-----------------+-----------------+
| USB线*1         | USB线*1         | 18              | 18650电池*2     |
|                 |                 | 650双节电池盒*1 | （电池自配）    |
+-----------------+-----------------+-----------------+-----------------+
...
.. image:: media/f9c8f6d733ac2cc54dbfb5030f04afb8.png
...
.. image:: media/247e252bab51cf65a1cacae0db0869ec.png
...
.. image:: media/2a0b40b983f1aead31d43e1662c4257e.png
...
+-----------------+-----------------+-----------------+-----------------+
| keyes UNO R3    | Keyes brick     | keyes           | HC-SR0          |
| for arduino     | L298P           | 草帽            | 4超声波传感器*1 |
| 开发板*1        | 电机驱动扩展板  | LED白发红模块*1 |                 |
|                 | V1*1            |                 |                 |
+=================+=================+=================+=================+
...
原理：看超声波的图可知，像是有两个眼睛，其一边是发射超声的，一边是接收超声波的，然后检测从发射遇到障碍物返回被接收到所需的时间t，再根据声音在空气中的传播速度大概是343m/s，\ **距离
= 速度 / 时间**
...
.. image:: media/d659ce5f14906b5e8744fca2f359986d.png
...
  上传好测试代码到开发板，打开串口监视器，设置波特率为9600，我们可以看到超声波模块显示的距离，单位是厘米。用手阻挡超声波模块，我们看到显示距离的数值变小了。

.. image:: media/ff0cd233b65f57153a09458fd5c3c258.png
...
.. image:: media/5c65aebcb35dd1c128148fa651b8b6a4.png
...
.. image:: media/817764a6b04fc6fff7ceb939f94224d5.jpeg

  红外遥控在日常生活中随处可见，它被用来控制各种家电，如电视、音响、录影机和卫星信号接收器。红外遥控是由红外发射和红外接收系统组成的，也就是一个红外遥控器和红外接收模块和一个能解码的单片机组成的。 

.. image:: media/05e43825a247a1d90a5d6d8509a6d14f.png
...
.. image:: media/9dcb924eb37101aa05b6f21d317fbc3b.png
...
.. image:: media/17b787d350e8882a03f0c31eeee3c9f2.png
...
+-----------------------+-----------------------+-----------------------+
| keyes UNO R3 for      | Keyes brick L298P     | keyes                 |
| arduino 开发板x1      | 电机驱动扩展板 V1x1   | 草帽LED白发红模块x1   |
+=======================+=======================+=======================+
| |image37|             | |image38|             | |image39|             |
+-----------------------+-----------------------+-----------------------+
| keyes brick           | JMP-1 17键红外遥控x1  | USB线x1               |
| 红外接收传感器x1      |                       |                       |
+-----------------------+-----------------------+-----------------------+
| |image40|             | |image41|             | |image42|             |
+-----------------------+-----------------------+-----------------------+
| 3Pin 双母头杜邦线*1   | X                     | 18650双节电池盒*1     |
|                       | H2.54-3Pin+杜邦母双*1 |                       |
+-----------------------+-----------------------+-----------------------+
...
.. image:: media/463e4394ec9e12cd1ae22d69007843ce.png
...
.. image:: media/56ad2d8c8eb870420d507cd5d70efc4f.png
...
.. image:: media/82ed4865d569b677e3185e25e76d39ca.png
...
  在这里，我们将学习HM-10 BLE 4.0。
HM-10是一种随时可用的蓝牙4.0模块。该模块用于建立无线数据通信，是使用德州仪器（TI）的CC2540或CC2541蓝牙低功耗（BLE）片上系统（SoC）设计的。
...
+-----------------+-----------------+-----------------+-----------------+
| keyes UNO R3    | Keyes brick     | keyes           | Keyes           |
| for arduino     | L298P           | 草帽            | Bluetooth-4.0   |
| 开发板*1        | 电机驱动扩展板  | LED白发红模块*1 | 蓝牙4.0 V2*1    |
|                 | V1*1            |                 |                 |
+=================+=================+=================+=================+
...
+-----------------+-----------------+-----------------+-----------------+
| |image50|       | |image51|       | |image52|       | |image53|       |
+-----------------+-----------------+-----------------+-----------------+
...
.. image:: ./media/593d03b3e63313f2a5f2b93e2048075c.png
...
.. image:: media/f5f1708bf257c88dbcfc1782491f073f.png
...
.. image:: media/b2727ac1d2ad70764616d61b5b7b1fb0.png

  3、点击上图图标，进入APP，显示如下图。

.. image:: media/8051e12c71828866255ca9a92662b2ae.jpg

  4、REV4板上传代码成功后，连接蓝牙，上电后，蓝牙模块上LED闪烁。点击APP\ |image54|\ 图标，搜索到蓝牙，显示如下图。

.. image:: media/0fcbbb8c87d427a4bfa12285c0eccde7.jpg

  5、点击连接，蓝牙连接成功，显示如下图，蓝牙模块上LED变为常亮。

.. image:: media/2e2791e116b1c5b0d2933a197e6e97ff.jpg
...
.. image:: media/29c5a8fc3e39ca5259f31baf812ef041.png

  2、点击搜索，搜索keyes，下载搜索到的keyes BT car。

.. image:: media/04e623ed0a9df1c1b7e55cab2a945589.png

  3、打开keyes BT car。

.. image:: media/75e98db6a123422655b70d481c0c0e32.png

  4、开启手机蓝牙，点击左上角的connect按钮，进行蓝牙搜索和连接。

.. image:: media/5e92657dcaeaebeccbe0ef212daf04ec.png

  5、点击桌面小车的\ |image55|\ 图片按钮，进入控制桌面小车的界面
...
上面的项目，我们讲解了蓝牙接收到手机发送的信号并且在开发板的串口显示出来，比如我们按下\ |image56|\ ，然后我们就会接收到‘B’，当我们松开的时候又接收到‘S’。那接下来我们就要想一下了，我们可以利用接收到的信号去做一些事情吗，答案是肯定的，我们这里就利用手机发送的命令去打开或者关闭一个LED灯。看接线图，在D9脚接了一个LED。

.. image:: ./media/6c4c16f0a0f95f88287e05f0e935ef15.png
...
  上传代码完成后，点击手机APP上\ |image57|\ 以控制LED。当您按下发送``B’‘时，LED将打开，而当您松开发送``S’’时，LED将关闭。

.. image:: media/26cb7b1cfaa0d52caa58cb12cc782562.png
...
.. image:: media/df0709b0a0ce857a0a5a7a441d3708e6.png

.. image:: media/7fc06405e02b8a7a50bce7750e12f55e.png
...
+-----------------------+-----------------------+-----------------------+
| keyes UNO R3 for      | Keyes brick L298P     | 4.5V 200转/分         |
| arduino 开发板*1      | 电机驱动扩展板 V1*1   | 单轴减                |
|                       |                       | 速箱+双头轴马达+250MM |
|                       |                       | PH2.0mm-2P线材*4      |
+=======================+=======================+=======================+
...
.. image:: ./media/54fdab2761f7430cbedade518fbf9c55.png
...
.. image:: media/426284f651896c8c6dfc5f4d2974b3ab.jpg
...
+-------------+-------------+-------------+-------------+-------------+
| keyes UNO   | Keyes brick | USB线*1     | 18650双     | 18650电池*2 |
| R3 for      | L298P       |             | 节电池盒*1  | （          |
| arduino     | 电机        |             |             | 电池自配）  |
| 开发板*1    | 驱动扩展板  |             |             |             |
|             | V1*1        |             |             |             |
...
.. image:: media/8acfd52a64b8ebce33fad4fe207d5ea2.png
...
.. image:: media/2f63c317b84c809c96550cc1e204664e.png
...
.. image:: media/94c238ce64374fdababbfd3dc1738eec.png

   ③地址命令设置，有如下图地址可以选

  我们示例程序中选了第一个00H，其二进制1100 0000对应的十六进制是0xc0

.. image:: media/9acaaab02c25385cf3dda86947424f15.png
...
.. image:: media/dcd8dbbdfbb5ce9c25e3269a7eb89364.png
...
  设置时，我们需要把一个图案转换成1组16个的16位数据，这里就需要用到一个取模软件,这个软件已放入资料文件夹中。使用时打开\ |image73|\ 图标，显示如下图。

.. image:: media/c8281eddce26ce57970d5f152aabe2a4.png

  点击\ |image74|\ 这个图标新建图案，根据显示屏规格，设置宽度为16，高度为8，如下图。

.. image:: media/a1c97576e9772caf16f3947709a62a37.png

  初始时发现格点不大，不方便设置，我们可以通过设置模拟动画，设置格点大小，点击如下图。

.. image:: media/655611122bc3f5fd2ee8e5176c3847ca.png

  一直鼠标左键点击\ |image75|\ ，就可以一直放大格点了。

  放大后，我们就可以通过用鼠标点击白色区域，设置显示图案了。

.. image:: media/07834404e360fb5a561cab18d039e04d.png

  设置时，鼠标点击（左右键都可以）白色格点，变为黑色；再点击黑色格点，变为白色。黑色代表该格点显示亮起，白色代表格点不显示。显示屏最多能设置16*8个点显示。设置笑脸显示如下图。

.. image:: media/fa3de1661e05dccdbbd105a9a183b85d.png

  设置参数设置，选择其他选项，设置如下图。设置完成点击\ |image76|\ 。

.. image:: media/d24b1b5674a1ac003349621db4224f81.png

.. image:: media/05779d4455696252a12c0df7e184465b.png

  设置取模方式，选择C51格式选择如下图。

.. image:: media/a18329dc5ddfc4d9099b80f62f59d2ae.png
...
.. image:: media/d3aa71456797375083d34feeffe11d66.png
...
.. image:: media/678879eee681ffef1e455c9c9b8461f3.png
...
   在keyes开发板上传代码成功，按照接线图接线，拨码开关拨打到右端上电后，看一下，我们的显示屏上是不是显示了一个笑脸。

.. image:: media/d6e69a3fd6ba16c6dc0e34d9cbafdf48.png
...
  我们利用刚刚学到的取模工具，\ `http://dotmatrixtool.com/# <http://dotmatrixtool.com/>`__,让点阵循环显示开始图案，前进图案，停止图案，然后清除图案，时间间隔为2000毫秒。
...
.. image:: media/62934fafbccedab49b5e8e62ab52d595.png

.. image:: media/b676867d8d9b4f1973244a255f46f1fc.png

.. image:: media/71b8d2089f39a655481e98faceb06136.png
...
| 到黑线且右循迹传感器没检测到黑线  |                                   |
+-----------------------------------+-----------------------------------+
| 左循迹传感器检测到黑线            | 后退（PWM设为200）                |
| 或者中循迹传感器检测              | 然后左旋转（PWM设为200）          |
| 到黑线或者右循迹传感器检测到黑线  |                                   |
+-----------------------------------+-----------------------------------+
...
.. image:: ./media/5e69157a55bb98d4e8eb53e0372367ac.png
...
  将驱动扩展板堆叠在UNO
R3板上，上传好代码，按照接线图接线，将拨码开关拨至ON端后，智能车能够沿着黑线行走。
...
.. image:: media/ef5ded98940f8f89b05df5a616a5602c.png
...
| -             | -                       | -                      |
+---------------+-------------------------+------------------------+
| 0<distance<20 | distance_l > distance_r | 向左转                 |
|               | 如果左边大于右边        |                        |
+---------------+-------------------------+------------------------+
| 0<distance<20 | distance_l<=distance_r  | 向右转                 |
|               | 如果左边不大于右边      |                        |
+---------------+-------------------------+------------------------+
| distance>=20  | 前进                    | 前进                   |
+---------------+-------------------------+------------------------+
...
.. image:: media/adacc5a207fc3bc8cbf8722346dd4d71.png
...
  将驱动扩展板堆叠在UNO
R3板上，上传好代码，按照课程三接线图接线，将拨码开关拨至ON端后，智能车能够自动避开障碍物行走。
...
+-----------------------+-----------------------+-----------------------+
| 按键：\ |image81|     | 键值：FF629D          | 状态：前进            |
+=======================+=======================+=======================+
| 按键：\ |image82|     | 键值：FFA857          | 状态：后退            |
+-----------------------+-----------------------+-----------------------+
| 按键：\ |image83|     | 键值：FF22DD          | 状态：左转            |
+-----------------------+-----------------------+-----------------------+
| 按键：\ |image84|     | 键值：FFC23D          | 状态：右转            |
+-----------------------+-----------------------+-----------------------+
| 按键：\ |image85|     | 键值：FF02FD          | 状态：停止            |
+-----------------------+-----------------------+-----------------------+
...
  **接线注意**\ ：由于红外接收传感器输入的数字信号，将红外接收传感器模块用导线连接到电机驱动扩展板上的G、V、A1，A、B两组电机分别对应的连接到堆叠在UNO
R3板上的电机驱动扩展板上的接口A和接口B，电源接到BAT接口。

.. image:: media/b474b130a3a68000c8ebcf57cf501c01.png
...
|                       | 能：进入蓝牙控制界面  |                       |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image88|        | 功能：断开蓝牙连接    |                       |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image89|        | 控制字                | 功能：按下，          |
|                       | 符：按下：F；松开：S  | 小车前进；松开就停止  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image90|        | 控制字                | 功能：按下，          |
|                       | 符：按下：B；松开：S  | 小车后退；松开就停止  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image91|        | 控制字                | 功能：按下，小        |
|                       | 符：按下：L；松开：S  | 车左旋转；松开就停止  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image92|        | 控制字                | 功能：按下，小        |
|                       | 符：按下：R；松开：S  | 车右旋转；松开就停止  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image93|        | 控制字                | 功能：点              |
|                       | 符：按下：a；松开：S  | 击，加速，最大加到255 |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image94|        | 控制字                | 功能：                |
|                       | 符：按下：d；松开：S  | 点击，减速，最小减到0 |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image95|        | 控制字符：            | 功                    |
|                       |                       | 能：点击一下开启手机  |
|                       |                       | 方向感应控制，再点击  |
|                       |                       | 一下退出方向感应控制  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image96|        | 控制字符：点击        | 功能：开启避          |
|                       | 发送：Y,再次电机发送S | 障功能，再次点击退出  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image97|        | 控制字符：点击        | 功能：开启循          |
|                       | 发送：X,再次电机发送S | 线功能，再次点击退出  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image98|        | 控制字符：点击        | 功能：开启超声波跟    |
|                       | 发送：U,再次电机发送S | 随功能，再次点击退出  |
+-----------------------+-----------------------+-----------------------+
| 按钮:|image99|        | 控制字符：点击        | 功能：开启画地为      |
|                       | 发送：G,再次电机发送S | 牢功能，再次点击退出  |
+-----------------------+-----------------------+-----------------------+
...
.. image:: media/79f58382d215a2e716b7da3b204cc709.png
...
按下\ |image100|\ 按钮，小车前进；按下\ |image101|\ 按钮，小车后退；按下\ |image102|\ 按钮，小车左旋转；按下\ |image103|\ 按钮，小车右旋转；点击一下\ |image104|\ 按钮，开启手机方向感应控制，再点击一下\ |image105|\ 按钮，退出方向感应控制。
...
  按下\ |image106|\ 按钮，小车加速；按下\ |image107|\ 按钮，小车减速。
...
.. image:: ./media/e1c652df1af506b724d88fe6bb022512.png
...
  将驱动扩展板堆叠在UNO
R3板上，上传好代码，按照接线图接线，将拨码开关拨至ON端后，手机APP连接蓝牙成功后，我们就能用手机APP控制智能车运动了。我们可以通过按下对应按钮实现对应功能，通过停止钮来停止功能。

  **注意**\ ：利用安卓系统手机APP点击\ |image108|\ ，测试语音控制时，不能实现语音控制功能。

.. |image1| image:: media/85b2076096371fd9ca03671602b1e5ce.png
.. |image2| image:: media/31734302247e9a18efc688056a9c1019.png
.. |image3| image:: media/fa9659fdadb816de22363e180b01cc51.png
.. |image4| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image5| image:: media/31fb938502d3d519813c391569d6a3f3.png
.. |image6| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image7| image:: media/a86a5db2b0af8b35a94356bc47796b03.jpg
.. |image8| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image9| image:: media/67417bd98f12bffd0352f76063e5abbd.png
.. |image10| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image11| image:: media/31fb938502d3d519813c391569d6a3f3.png
.. |image12| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image13| image:: media/a86a5db2b0af8b35a94356bc47796b03.jpg
.. |image14| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image15| image:: media/67417bd98f12bffd0352f76063e5abbd.png
.. |image16| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image17| image:: media/31fb938502d3d519813c391569d6a3f3.png
.. |image18| image:: media/4f3f0b0638fcb1a64af48afdc3740309.png
.. |image19| image:: media/f5dc1ad0f10e043c326aefa252a82575.png
.. |image20| image:: media/a86a5db2b0af8b35a94356bc47796b03.jpg
.. |image21| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image22| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image23| image:: media/67417bd98f12bffd0352f76063e5abbd.png
.. |image24| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image25| image:: media/0fd7b4148500d7edd1becf5acfeac337.png
.. |image26| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image27| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image28| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
...
.. |image32| image:: media/7fcf3f572a14fd99b8439d3c1604ea62.png
.. |image33| image:: media/146f0fc8a39a7d767675ff130236d967.png
.. |image34| image:: media/a86a5db2b0af8b35a94356bc47796b03.jpg
.. |image35| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image36| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image37| image:: media/67417bd98f12bffd0352f76063e5abbd.png
.. |image38| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image39| image:: media/31fb938502d3d519813c391569d6a3f3.png
.. |image40| image:: media/a6b01539a553d69acfd8b9c5a5e1d500.png
.. |image41| image:: media/2a93c4291fa6f2a2c8d2cf50babbcc55.png
.. |image42| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image43| image:: media/a86a5db2b0af8b35a94356bc47796b03.jpg
.. |image44| image:: media/c704647f0d48171a099121537d437794.png
...
.. |image48| image:: media/31fb938502d3d519813c391569d6a3f3.png
.. |image49| image:: ./media/676ec7614a03a6e2546cee45858c9b16.png
.. |image50| image:: media/a86a5db2b0af8b35a94356bc47796b03.jpg
.. |image51| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image52| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image53| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image54| image:: media/93bbea5cdfbd12d0d11a9a8e85d33b0e.png
.. |image55| image:: media/1ec777d4324b73a9faa0973f0beb90f1.png
.. |image56| image:: media/4294a850df600cd901f59b2a02fb3237.png
.. |image57| image:: media/2c775d76241235f2d35df8a104b4ea41.png
.. |image58| image:: media/67417bd98f12bffd0352f76063e5abbd.png
.. |image59| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image60| image:: media/b5951bc181e1ac91cfb214789dfa709c.png
.. |image61| image:: media/b5951bc181e1ac91cfb214789dfa709c.png
.. |image62| image:: media/b5951bc181e1ac91cfb214789dfa709c.png
.. |image63| image:: media/b5951bc181e1ac91cfb214789dfa709c.png
.. |image64| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image65| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image66| image:: media/67417bd98f12bffd0352f76063e5abbd.png
.. |image67| image:: media/3dca1bdd1d1420c1d12b16cbf52fee00.png
.. |image68| image:: media/b54b3d7da383ff2147f8a15a658d6102.jpg
.. |image69| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image70| image:: media/c5bf59a8e5cdded95c02334369ab6fdd.png
.. |image71| image:: media/4b3b276b9242a386ec26a38cc48b6401.png
.. |image72| image:: media/b289bdda58092c91ab7bbcff4eac9aa1.png
.. |image73| image:: media/c7e1f4b0440706c0cdb7e745ac29844a.png
.. |image74| image:: media/692d9479a8801057b02c0b3f26861a15.png
.. |image75| image:: media/87dbff178a12ce43c6c6f40877483de5.png
.. |image76| image:: media/b8373eb149df42e9b559824a8f1ccb82.png
.. |image77| image:: media/bba60b6abb9d6a964e9213222448d3a2.png
.. |image78| image:: media/609a8b068bb8f95a328718bac3fba986.png
.. |image79| image:: media/e7fbd5f40b576712264670f1da25e73a.png
.. |image80| image:: media/aae33c656475a9d989dddf5fc2f6b967.png
.. |image81| image:: media/b11dc5ffa6cccebc6088e5d557d76daf.png
.. |image82| image:: media/ae8110034aacb083151cfd882ee599ba.png
.. |image83| image:: media/bce9cba2c6d2465fbcce570ad4210eba.png
.. |image84| image:: media/ad907a618af86f30d52986bbbd57ba76.png
.. |image85| image:: media/9716a4ed61a4064d2f47a7b73eccaf87.png
.. |image86| image:: ./media/4afc5a1a6e3faed00f02d4743268bd53.png
.. |image87| image:: media/640f7010c0d1101faa37b50d1143572e.png
.. |image88| image:: media/b7596998206539ed470a302a0f12b747.png
.. |image89| image:: media/6223550c3d79e8e0f594ca15c5238cc8.png
.. |image90| image:: media/fb1772180f8aa36921d330c3e34a2edb.png
.. |image91| image:: media/5caffc69e999fb670bafdaf2b5cccd6b.png
.. |image92| image:: media/1ceac7609237f50a37c963a342475c38.png
.. |image93| image:: media/049343f587e0e7cf19fe8b665d735321.png
.. |image94| image:: media/264f77cce6018584b54f46676fee4247.png
.. |image95| image:: media/b2f9ad09c7912a461fff0af86298346e.png
.. |image96| image:: media/5aac2a167009fec900868ee75374b11a.png
.. |image97| image:: media/094b1761810aaed0e4ee1f466ea5f00b.png
.. |image98| image:: media/8eaf85628b37873183a2940fe00d7b93.png
.. |image99| image:: media/67ed8e9457f9b71d3ce93163dc3ce1e8.png
.. |image100| image:: media/7116d72a22a699991156feac4746ae2a.png
.. |image101| image:: media/0e99537a6cf32884ec107963e5107298.png
.. |image102| image:: media/e2dba3bd895f7d39fac137d74ff1b834.png
.. |image103| image:: media/3149880e1a7d4b1d61a43d586caade5a.png
.. |image104| image:: media/aa34fbcb6fb84503689c22e567e15881.png
.. |image105| image:: media/aa34fbcb6fb84503689c22e567e15881.png
.. |image106| image:: media/b2d6cb9c7c86786d8a5067ed7536dd31.png
.. |image107| image:: media/f70a0831084cf8578de1c3d256453480.png
.. |image108| image:: media/07774c50588977f9ab9398976b2be309.png
